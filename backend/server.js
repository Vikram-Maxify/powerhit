...
// =====================================================
// BET ROUTES (30s and 1m games only)
// =====================================================
...
const startServer = async () => {
  try {
    ...
    // Initialize initial game periods if not exists
    ...
    // =====================================================
    // CRON JOBS - 30s and 1m games only
    // =====================================================

    // 30 Second Game (wingo10)
    let lastRun30 = 0;
    setInterval(async () => {
      const now = Date.now();
      if (now - lastRun30 >= 30000) {
        lastRun30 = now;
        try {
          const period = await Wingo.findOne({ status: 0, game: "wingo10" })
            .sort({ _id: -1 })
            .limit(1);
    ...
    setInterval(async () => {
      ...
      if (now - lastRun1 >= 60000) {
        lastRun1 = now;
        try {
          const period = await Wingo.findOne({ status: 0, game: "wingo" })
            .sort({ _id: -1 })
            .limit(1);
    ...
    setInterval(async () => {
      ...
      if (now - lastRunTrx >= 60000) {
        lastRunTrx = now;
        try {
          const period = await Wingo.findOne({ status: 0, game: "trx" })
            .sort({ _id: -1 })
            .limit(1);
    ...
    // Also run commission immediately on startup
    setTimeout(async () => {
      try {
        await betController.tradeCommission();
