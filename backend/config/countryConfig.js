const countries = {
  ...
  canada: {
    ...
    GameCount: require("../models/canada/CanadaGameCount"),
    GamePool: require("../models/canada/CanadaGamePool"),
    PowerballDivision: require("../models/canada/CanadaPowerballDivision"),
    PowerballResult: require("../models/canada/CanadaPowerballResult"),
  },
};

function getCountryContext(req) {
  const raw = req.params?.country || req.body?.country || req.query?.country;
  const country = String(raw || "").trim().toLowerCase();
  const context = countries[country];
  if (!context) {
    const error = new Error("Unsupported country. Use india, uae, nepal, pakistan, australia or canada.");
...
module.exports = { countries, getCountryContext };
